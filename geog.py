...
BASE_URL = "https://carleton.ca/geography/staff-faculty-2/"
...
def get_person_blob_info(person_article: Tag) -> dict:
    ...
    try:
        ...
        for title in titles:
            # check if title contains the word "research"
            if 'current research' or 'research interest' in title.text.lower():
                big_data = []
                next_el = title.find_next_sibling()
                while next_el:
                    # exclude certain div containers with classes
                    if next_el.name == 'div' and ('people__photo' in next_el.get('class', []) or 'people__details' in next_el.get('class', []) or 'content__video' in next_el.get('class', [])):
                        break
...
def main():
    ...
    with open('geog.json', 'w', encoding="utf-8") as f:
        faculty_data = []
