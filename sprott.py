...
BASE_URL = "https://sprott.carleton.ca/directory/#faculty"
...
def get_person_table_info(table_info: Tag):
    ...
    try:
        # Find the table
        dl_list = table_info.find_all('dl')
        for dl in dl_list:
            # Get the dt and dd cells
            dt_cells = dl.find_all('dt')
            dd_cells = dl.find_all('dd')
            # Store the result in the dictionary
            for i in range(len(dt_cells)):
                result.update({dt_cells[i].text: dd_cells[i].text})
...
def get_person_info(url: str) -> {}:
    ...
    logger.debug(url)
    person_article = BeautifulSoup(resp.content, 'html.parser')

    # Find the divs with the person details and picture, which will be further used to get the data
    person_detail = person_article.find("div", {"class": "details__meta"})
    person_picture = person_article.find("div",{"class": "details__aside"} )
    page_header = person_article.find("header")


    # Get the name, department, title, picture, contacts and research info
    result.update({"name": page_header.find('h1').text})
    # get the person's department
    result.update({"department":"Sprott School of Business"})
...
def get_faculties_list_with_links(soup: Tag) -> dict[str, Tag]:
    ...
    for h3_tag in h3_tags:
        if h3_tag.text == "Faculty":
            section_tag = h3_tag.find_next("div")
...
def main():
    ...
    soup = BeautifulSoup(resp.content, 'html.parser', parse_only=SoupStrainer(id="content") )
    ...
    with open('sprott.json', 'w', encoding="utf-8") as f:
        faculty_data = []
